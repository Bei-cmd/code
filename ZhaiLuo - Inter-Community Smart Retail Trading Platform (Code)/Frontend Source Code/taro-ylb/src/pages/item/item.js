...
class Item extends Component {
  ...
  handleAdd = () => {
    ...
    const isSingleSpec = skuSpecList.every(
      spec => spec.skuSpecValueList.length === 1
    );
  ...
  render() {
    const { itemInfo, recommend } = this.props;
    ...
    return (
      <View className="item">
        <ScrollView scrollY className="item__wrap" style={{ height }}>
        ...
        <View className="item__footer">
          <Footer data={itemInfo} onAdd={this.handleAdd} />
        </View>
      </View>
    );
  }
